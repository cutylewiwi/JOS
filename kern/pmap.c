...
boot_alloc(uint32_t n)
{
    ...
    // step1: check n
    // step2: alloc a chunck
    // if return, always return nextfree when boot_alloc is called
    
    if (n >= 0){
        result = (void *) nextfree;
        nextfree = ROUNDUP(nextfree + n, PGSIZE);
        
        if (PADDR (nextfree) > npages * PGSIZE){	// overflow the physics address space
            nextfree = (char *) result;
            panic("kern/pmap.c: out of memory\n");
        }
    }
...
page_free(struct PageInfo *pp)
{
    ...
    // panic rountine
    if (pp->pp_link){
        cprintf("error page: %p\n", page2kva(pp));
        panic("page_free: page to free has non-null pp_link\n");
    }
    
    if (pp->pp_ref){
        panic("page_free: page to free has non-zero pp_ref\n");
    }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (!(*pde & PTE_P)){
        ...
        // create a new page for page table
        ...
        // set up pde
        * pde = page2pa(pt_page) | PTE_W | PTE_U | PTE_P;
    }
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    for (va_iterator = (void *)va; va_iterator < (void *)(va + size); va_iterator += PGSIZE, pa += PGSIZE){
        // get page table entry
   	   	pte = pgdir_walk(pgdir, va_iterator, 1);
        
   	   	if (!pte){
            panic("boot_map_region: unable to alloc page for page table\n");
        }
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
    ...
    if (!pte                    // no page table present
        || !(*pte & PTE_P)){    // no page mapped
        return NULL;
    }
...
page_remove(pde_t *pgdir, void *va)
{
    // Fill this function in
    pte_t * pte;
    struct PageInfo * mapped_page = page_lookup(pgdir, va, &pte);
    
    // no mapped address, do nothing
    if (!mapped_page){
        return;
    }
